<?php
	...
	$query2=" order by upvotes desc limit 25";
	echo '<script>console.log(\''.$query.$query2.'\');</script>';
	$query.=$query2;

	// $query="select * from course_tutorials order by upvotes desc limit 25";
	...
	$html = str_replace(array("{{row}}","{{title}}"),array( $ans,"Popular Tutorials"), $html);
	echo $html;
